import{a as ie,b as re,c as L,d as X}from"./chunk-WP4USC75.js";import{a as ee,c as se}from"./chunk-4NYNH44B.js";import{a as W}from"./chunk-ST53LDJE.js";import{a as z,g as oe,n as te}from"./chunk-DF4IVW5S.js";import{g as j,h as N,k as D,n as J}from"./chunk-YCNW34TK.js";import{a as M}from"./chunk-P6IECSNE.js";import{b as S,c as I,d as E,e as f,f as O}from"./chunk-KN6GOQRR.js";import{h as d,j as R,k as C,m as Q,o as K,p as $,q as P,r as V,s as F,t as A,u as Y,w as _}from"./chunk-K2UKPH6Q.js";var he=/^[^/]+/,me=/^(:\w|\()/,fe=/:\w|\(/,$e=/^(?::(\w+))?(?:\(([^)]+)\))?/,q=class{constructor(s){this.root=this.createNode(),s&&Object.entries(s).forEach(o=>this.add(...o))}createNode({regex:s,param:o,handler:i}={}){return{regex:s,param:o,handler:i,children:{string:{},regex:{}}}}add(s,o){return this.parseOptim(s,o,this.root),this}parseOptim(s,o,i){if(fe.test(s))this.parse(s,o,i);else{let t=i.children.string[s];t?t.handler=o:i.children.string[s]=this.createNode({handler:o})}}parse(s,o,i){if(me.test(s)){let t=s.match($e);if(t){let r=i.children.regex[t[0]];r||(r=i.children.regex[t[0]]=this.createNode({regex:t[2]?new RegExp("^"+t[2]):he,param:t[1]})),t[0].length===s.length?r.handler=o:this.parseOptim(s.slice(t[0].length),o,r)}}else{let t=s[0],r=i.children.string[t];r||(r=i.children.string[t]=this.createNode()),this.parse(s.slice(1),o,r)}}find(s){return this.findOptim(s,this.root,{})}findOptim(s,o,i){let t=o.children.string[s];return t&&t.handler!==void 0?{handler:t.handler,params:i}:this.findRecursive(s,o,i)}findRecursive(s,o,i){let t=o.children.string[s[0]];if(t){let r=this.findRecursive(s.slice(1),t,i);if(r)return r}for(let r in o.children.regex)if(t=o.children.regex[r],t.regex){let n=s.match(t.regex);if(n){if(n[0].length===s.length&&t.handler!==void 0)return t.param&&(i[t.param]=decodeURIComponent(n[0])),{handler:t.handler,params:i};{let a=this.findOptim(s.slice(n[0].length),t,i);if(a)return t.param&&(i[t.param]=decodeURIComponent(n[0])),a}}}return null}},ne=q;var ge=new ne({"/:napp(\\+{1,3}\\w+):appPath($|\\/.*)":{},"/napp-updates":{mount:"system-views",path:"/napp-updates",tag:"napp-updates",loadModule:()=>import("./napp-updates-26OEHUDL.js")},"/settings":{mount:"system-views",path:"/settings",tag:"a-settings",loadModule:()=>import("./settings-TECB25EO.js")}}),U=ge;function Z(){return F("useScrollbarConfig",()=>{let e=document.createElement("div");e.style.cssText=`
      position: absolute;
      top: -9999px;
      width: 100px;
      height: 100px;
      overflow: scroll;
      visibility: hidden;
    `,document.body.appendChild(e);let s=e.offsetWidth-e.clientWidth;document.body.removeChild(e);let o=s>0;return{width:s,hasClassic:o,hasOverlay:!o,className:o?"classic-scrollbar":"overlay-scrollbar"}})}d("pointerupInterceptor",function(){let{props:{isOpen$:e,isOpenedByLongPress:s}}=this;if(!s)return;let o=$(()=>({isFirstRun$:!0,dialogRef$:null,shouldOpen$(){let i=e();return this.isFirstRun$.get(!1)?(this.isFirstRun$.set(!1),!1):i},onPointerUp(){requestIdleCallback(()=>this.dialogRef$().hidePopover(),{timeout:150})}}));return _(({track:i})=>{i(()=>o.shouldOpen$())&&o.dialogRef$().showPopover()},{after:"rendering"}),this.h`<div
    popover
    ref=${o.dialogRef$}
    ...
    style=${`
      ...
      height: 100%;
    `}
  />`});d("aMenu",function(){let e=$({id$:this.props.id$||this.props.id||"a"+Math.random().toString(36).slice(2)+Math.random().toString(36).slice(2),dialogRef$:null,render:this.props.render,shouldAlwaysDisplay$:this.props.shouldAlwaysDisplay$??this.props.shouldAlwaysDisplay??!1,isOpen$:this.props.isOpen$,close:this.props.close,afterClose:this.props.afterClose,style$:this.props.style$??this.props.style??"",anchorRef$:this.props.anchorRef$,fallbackPositioningStyle$:"",fallbackOffset$:this.props.fallbackOffset??{}}),s=$(()=>({isOpen$:e.isOpen$,isOpenedByLongPress:this.props.isOpenedByLongPress??!1}));return _(({track:o})=>{let i=o(()=>e.isOpen$.get()),t=o(()=>e.anchorRef$());!i||!t||CSS.supports("position-anchor","--test")||(e.fallbackPositioningStyle$(`
      & {
        visibility: hidden;
      }
    `),setTimeout(()=>{requestAnimationFrame(()=>{let r=t.getBoundingClientRect(),n=e.dialogRef$().getBoundingClientRect(),a=window.innerWidth>window.innerHeight,p=6,l,y;if(a)l=Math.max(p,r.left-n.width-p),y=r.top;else{l=r.left;let m=n.height>0?n.height:100;y=Math.max(p,r.top-m-p)}let h=e.fallbackOffset$(),c=(a?h.landscape:h.portrait)||{};l+=c.x||0,y+=c.y||0,e.fallbackPositioningStyle$(`
          & {
            left: ${l}px;
            top: ${y}px;
            right: auto;
        ...
        `)})},100))},{after:"rendering"}),_(({track:o})=>{o(()=>e.isOpen$.get())?e.dialogRef$().showPopover():e.dialogRef$().hidePopover()},{after:"rendering"}),this.h`
    <dialog
      id=${e.id$()}
      ref=${e.dialogRef$}
      data-name='menu'
  ...
  />`});d("toolbar-more-menu",function(){let{isHidden$:e}=P("toolbarState",{isHidden$:!1}),{isOpen$:s,anchorRef$:o}=$({isOpen$:!1,anchorRef$:null}),i=M(),{session_unread_appUpdateCount$:t}=S(localStorage),r=$({isOpen$:s,anchorRef$:o,close:()=>s.set(!1),fallbackOffset:{portrait:{x:-119,y:5},landscape:{x:-17,y:-15}},style$:()=>{let n=`& {
      ...
      `;return`& { ${CSS.supports("position-anchor","--test")?n:a} ${p} } &:not(:popover-open) { display: none; }`},render:()=>this.h`
      <div id='scope_toolbar_more_menu_content'>
        ...
        <div onclick=${()=>{i.pushState({},"","/settings"),s.set(!1)}}>
          ...
          ${(t()??0)>0?this.h`<div class='badge-dot'></div>`:""}
        </div>
      </div>
    `});return this.h`
    <div
    ...
    >
      ...
      ${(t()??0)>0?this.h`<div class='more-menu-badge'></div>`:""}
    </div>
    <a-menu props=${r} />
  `});d("toolbar-restore-button",function(){let{isHidden$:e}=P("toolbarState");return this.h`
    <div
  ...
  `});d("aScreen",function(){re(),H();let{isSystemRoute$:e}=G(),s=S(localStorage).config_isSingleWindow$,{isHidden$:o}=P("toolbarState",{isHidden$:!1}),i=K(()=>`
    /* @scope { */
    #screen {
      &${I.defaultTheme}
    ...
    #unified-toolbar {
      ...
      @media (orientation: portrait) {
        min-height: ${o()?"0px":"50px"};
        height: ${o()?"0px":"50px"};
        opacity: ${o()?"0":"1"};
      }
      @media (orientation: landscape) {
        flex-direction: column;
        min-width: ${o()?"0px":"50px"};
        width: ${o()?"0px":"50px"};
        opacity: ${o()?"0":"1"};
      }
      /**/
  ...
  `),t=Y("unifiedToolbarRef",null);return this.h`
    <div id="screen" class=${{"multi-window":!s(),[E.defaultTheme]:!0}}>
      <style>${i()}</style>
      <div id='workspaces'>
  ...
  `});d("aWindows",function(){let{session_openWorkspaceKeys$:e}=S(localStorage),s=C([]);_(({track:i})=>{let t=i(()=>e());s(r=>r.concat(t.filter(n=>!r.includes(n))))});let o=K(()=>e().reduce((i,t,r)=>({...i,[t]:r+1}),{}));return this.h`
  ...
  `});d("workspaceWindow",function(){let e=S(localStorage),{[`session_workspaceByKey_${this.props.workspaceKey}_openAppKeys$`]:s}=e,o=C([]);_(({track:t})=>{let r=t(()=>s());o(n=>n.concat(r.filter(a=>!n.includes(a))))});let i=K(()=>(s()??[]).reduce((t,r,n)=>({...t,[r]:`${this.props.mruRankByWsKey$()[this.props.workspaceKey]}-${n+1}`}),{}))();return this.h`
    ${o().map(t=>{let r=i[t];return this.h({key:t})`
      <app-window key=${t} props=${{appKey:t,wsKey:this.props.workspaceKey,mruRank:r}} />
      `})}
  `});d("appWindow",function(){let e=S(localStorage),{[`session_appByKey_${this.props.appKey}_id$`]:s,[`session_appByKey_${this.props.appKey}_visibility$`]:o,[`session_appByKey_${this.props.appKey}_route$`]:i,[`session_workspaceByKey_${this.props.wsKey}_userPk$`]:t}=e,r=K(()=>(t()||"")&&N(t(),50)),n=K(()=>D(s(),r())),a=K(()=>o()==="closed"),p=C(null),l=C("about:blank"),y=C(null),h=C("about:blank"),{cachingProgress$:c}=A("<napp-assets-caching-progress-bar>",{cachingProgress$:{}}),{requestVaultMessage:m}=X(),g=P("<permission-dialog>"),{requestPermission:b}=g,{openApp:u}=P("useAppRouter");if(_(async({track:w,cleanup:v})=>{let[x,k]=w(()=>[a(),p()]);if(x){c({}),p(null),h("about:blank"),y(null),l("about:blank");return}if(!k)return;let B=i()||"";B&&i("");let T=new AbortController;v(()=>T.abort()),await ie(r(),s(),n(),B,p(),y(),h,c,m,b,u,{signal:T.signal,isSingleNapp:!1}),l(`//${n()}.${window.location.host}/~~napp`)},{after:"rendering"}),!a())return this.h`
    <div
      style=${`
        background-color: ${f.colors.bg};
      `}
  ...
  `});d("unifiedToolbar",function(){let e=Z();return this.h`
    <style>${`
      /* @scope { */
      #unified-toolbar {
    ...
    <toolbar-app-list />
    <toolbar-more-menu />
  `});d("toolbarActiveAvatar",function(){return A("<a-menu>",{isOpen$:!1,anchorRef$:null,open(){this.isOpen$(!0)},close(){this.isOpen$(!1)},toggle(){this.isOpen$(e=>!e)}}),this.h`
    <toolbar-menu />
    <toolbar-avatar />
  `});d("toolbarMenu",function(){let e=S(localStorage),{session_openWorkspaceKeys$:s,session_workspaceKeys$:o}=e,{close:i}=A("<a-menu>"),t=L(),{requestVaultMessage:r}=X(),n=C({}),a=C({}),p=e.session_defaultUserPk$,l=K(()=>{let u=[],w={};o().forEach(x=>{let k=e[`session_workspaceByKey_${x}_userPk$`]();if(k!=null){let B=e[`session_accountByUserPk_${k}_profile$`](),T=e[`session_accountByUserPk_${k}_isLocked$`]();w[k]===void 0&&(w[k]=0),w[k]++,u.push({userPk:k,wsKey:x,profile:B,name:B?.name||B?.npub||k!==p()&&j(k)||"Default User",isLocked:T,index:w[k],totalCount:w[k]})}});let v={};return u.forEach(x=>{v[x.userPk]===void 0&&(v[x.userPk]=0),v[x.userPk]++}),u.forEach(x=>{x.totalCount=v[x.userPk]}),u}),y=K(()=>{let u=s()[0];return e[`session_workspaceByKey_${u}_userPk$`]()}),{disableStartAtVaultHomeWorkaroundThisTime:h}=P("vaultMessenger"),c=R(async(u,w,v)=>{if(u!==y()){let x=[...s()],k=[w,...x.filter(B=>B!==w)];e.session_openWorkspaceKeys$(k)}if(v){let x=`${u}-${w}`;n({...n(),[x]:!0}),a({...a(),[x]:null});try{let k=j(u),B=await r({code:"UNLOCK_ACCOUNT",payload:{pubkey:k}},{timeout:12e4,instant:!0});if(B.error||!B.payload?.isRouteReady)throw new Error(B.error?.message||"Failed to unlock account");i(),h(),t.open()}catch(k){a({...a(),[x]:k.message||"Error unlocking"}),setTimeout(()=>{a(B=>{let T={...B};return delete T[x],T})},3e3)}finally{n(k=>{let B={...k};return delete B[x],B})}}else i()}),m=R(()=>{i(),t.open()}),g=A("<a-menu>"),b=$({render:R(function(){return this.h`<div id='user-selection-menu'>
        <style>${`
          #user-selection-menu {
            display: flex;
            flex-direction: column;
        ...
        `}</style>
        ${l().map(u=>{let w=`${u.userPk}-${u.wsKey}`,v=n()[w],x=a()[w];return this.h({key:w})`<div
            class=${{"user-item":!0,active:u.userPk===y(),unlocking:v}}
            onclick=${()=>c(u.userPk,u.wsKey,u.isLocked)}
          >
        ...
        position: fixed;
        z-index: 1000;
      }`,...g});return this.h`<a-menu props=${b} />`});d("toolbarAvatar",function(){let e=S(localStorage),{session_openWorkspaceKeys$:s,session_workspaceKeys$:o}=e,i=K(()=>{let c=s()[0];return e[`session_workspaceByKey_${c}_userPk$`]()}),t=K(()=>{let c=i();return c?e[`session_accountByUserPk_${c}_isLocked$`]():!1}),r=K(()=>{let c=i(),m=s()[0];if(!c||!m)return{index:1,showBadge:!1};let g=0,b=0;for(let u of o())e[`session_workspaceByKey_${u}_userPk$`]()===c&&b++;for(let u of o())if(e[`session_workspaceByKey_${u}_userPk$`]()===c&&(g++,u===m))break;return{index:g,showBadge:b>1}}),{toggle:n,close:a,anchorRef$:p}=A("<a-menu>"),l=L(),y=K(()=>i()!==e.session_defaultUserPk$()||s().length>1);_(({track:c})=>{c(()=>y())||a()});let h=R(()=>{if(y())return n();l.open()});return this.h`<div
    ref=${p}
    onclick=${h}
    style=${`
  ...
  </div>`});d("toolbarAppList",function(){return A("<a-menu>",()=>({isOpenedByLongPress:!1,isOpen$:!1,open(){this.isOpen$(!0)},close(){this.isOpen$(!1)},app$:{key:""},toggleMenu(e){this.app$().key===e.key?(this.app$(e),this.isOpen$(o=>!o)):(this.close(),window.requestIdleCallback(()=>{this.app$(e),this.open()},{timeout:150}))}}),{isStatic:!1}),this.h`
    <toolbar-pinned-apps />
    <toolbar-unpinned-apps />
  `});d("toolbarPinnedApps",function(){let e=S(localStorage),{session_openWorkspaceKeys$:s}=e,o=K(()=>{let i=s()[0];return(e[`session_workspaceByKey_${i}_pinnedAppIds$`]()||[]).reduce((r,n,a)=>{let p=a+1;return e[`session_workspaceByKey_${i}_appById_${n}_appKeys$`]().forEach(l=>{r.push({appId:n,appKey:l,appIndex:p})}),r},[])});return this.h`${o().map(i=>this.h({key:i.appKey})`<toolbar-app-launcher key=${i.appKey} props=${i} />`)}`});d("toolbarUnpinnedApps",function(){let e=S(localStorage),{session_openWorkspaceKeys$:s}=e,o=K(()=>{let i=s()[0],t=(e[`session_workspaceByKey_${i}_pinnedAppIds$`]()||[]).length;return(e[`session_workspaceByKey_${i}_unpinnedAppIds$`]()||[]).reduce((n,a,p)=>{let l=p+1+t;return e[`session_workspaceByKey_${i}_appById_${a}_appKeys$`]().forEach(y=>{n.push({appId:a,appKey:y,appIndex:l})}),n},[])});return this.h`
    <app-launchers-menu />
    ${o().map(i=>this.h({key:i.appKey})`<toolbar-app-launcher key=${i.appKey} props=${i} />`)}
  `});d("appLaunchersMenu",function(){let e=A("<a-menu>"),s=S(localStorage),{requestConfirmation:o}=P("<confirmation-dialog>"),i=$(()=>({...e,openApp(){let{visibility:t,key:r,workspaceKey:n}=this.app$();if(t==="open")throw new Error("App is already open");this.close(),s[`session_appByKey_${r}_visibility$`]("open"),s[`session_workspaceByKey_${n}_openAppKeys$`]((a,p)=>{let l=a.indexOf(r);return l!==-1&&a.splice(l,1),a.unshift(r),a[p]=Math.random(),a})},bringToFirst(){let{visibility:t,key:r,workspaceKey:n}=this.app$(),a=s[`session_workspaceByKey_${n}_openAppKeys$`]();if(t!=="open")throw new Error("Can only bring to first when app is open");if(a[0]===r)throw new Error("App is already first");this.close();let p;s[`session_workspaceByKey_${n}_openAppKeys$`]((l,y)=>(p=l.indexOf(r),p>-1&&(l.splice(p,1),l.unshift(r),l[y]=Math.random()),l))},minimizeApp(){let{visibility:t,key:r,workspaceKey:n}=this.app$();if(t!=="open")throw new Error("Can only minimize an open app");this.close();let a;s[`session_appByKey_${r}_visibility$`]("minimized"),s[`session_workspaceByKey_${n}_openAppKeys$`]((p,l)=>(a=p.indexOf(r),a>-1&&(p.splice(a,1),p[l]=Math.random()),p))},closeApp(){let{visibility:t,key:r,workspaceKey:n}=this.app$();if(t==="closed")throw new Error("App is already closed");this.close(),s[`session_appByKey_${r}_visibility$`]("closed"),s[`session_workspaceByKey_${n}_openAppKeys$`]((a,p)=>{let l=a.indexOf(r);return l!==-1&&(a.splice(l,1),a[p]=Math.random()),a})},removeApp({isDeleteStep:t=!1}={}){let{id:r,key:n,workspaceKey:a}=this.app$(),p=s[`session_workspaceByKey_${a}_appById_${r}_appKeys$`]();if(!t&&p.length<=1)throw new Error("Cannot remove the last instance of an app");t||this.close(),s[`session_workspaceByKey_${a}_openAppKeys$`]((h,c)=>{let m=h.indexOf(n);return m!==-1&&(h.splice(m,1),h[c]=Math.random()),h});let l=p.filter(h=>h!==n);s[`session_workspaceByKey_${a}_appById_${r}_appKeys$`](l),s[`session_appByKey_${n}_id$`](void 0),s[`session_appByKey_${n}_visibility$`](void 0),s[`session_appByKey_${n}_route$`](void 0);let y=!1;for(let h of s.session_workspaceKeys$())if(y=(s[`session_workspaceByKey_${h}_appById_${r}_appKeys$`]()??[]).some(c=>c!==n),y)break;y||(s[`session_appById_${r}_icon$`](void 0),s[`session_appById_${r}_name$`](void 0),s[`session_appById_${r}_description$`](void 0),s[`session_appById_${r}_relayHints$`](void 0))},async maybeClearAppStorage(){let{id:t,workspaceKey:r}=this.app$(),n=s[`session_workspaceByKey_${r}_userPk$`](),a=s.session_workspaceKeys$().filter(h=>h!==r),p=!0,l=!0;for(let h of a)if(s[`session_workspaceByKey_${h}_appById_${t}_appKeys$`]()?.length>0&&(l=!1,s[`session_workspaceByKey_${h}_userPk$`]()===n)){p=!1;break}if(p){let h=N(n,50),c=D(t,h);await y(c)}l&&await(await te.create(t)).clearAppFiles();function y(h){let c=Promise.withResolvers(),m=document.createElement("iframe");m.style.display="none";let g,b=()=>{g&&clearTimeout(g),window.removeEventListener("message",w),document.body.removeChild(m)},u=`${window.location.protocol}//${h}.${window.location.host}`,w=v=>{v.origin===u&&(v.data.code==="DATA_CLEARED"&&(b(),c.resolve()),v.data.code==="DATA_CLEAR_ERROR"&&(b(),c.reject(v.data.error)))};return window.addEventListener("message",w),m.src=`${u}/~~napp#clear`,document.body.appendChild(m),g=setTimeout(()=>{b(),c.reject(new Error("Data clear timeout"))},5e3),c.promise}},async deleteApp(){try{await o({confirmText:"Delete"})}catch(t){t.code!=="DENIED_BY_USER"&&console.error(t);return}await this._deleteApp()},async _deleteApp(){let{id:t,workspaceKey:r}=this.app$();if(s[`session_workspaceByKey_${r}_appById_${t}_appKeys$`]().length!==1)throw new Error("Can only delete an app that has a single instance");this.removeApp({isDeleteStep:!0}),this.close(),s[`session_workspaceByKey_${r}_pinnedAppIds$`](a=>(a??[]).filter(p=>p!==t)),s[`session_workspaceByKey_${r}_unpinnedAppIds$`](a=>(a??[]).filter(p=>p!==t)),s[`session_workspaceByKey_${r}_appById_${t}_appKeys$`](void 0),await this.maybeClearAppStorage()},render:R(function(){let{openApp:t,bringToFirst:r,minimizeApp:n,closeApp:a,removeApp:p,deleteApp:l,app$:y}=i,{id:h,key:c,visibility:m,workspaceKey:g}=y(),b=s[`session_workspaceByKey_${g}_openAppKeys$`](),u=s[`session_workspaceByKey_${g}_appById_${h}_appKeys$`]();return this.h`<div id='scope_pfgf892'>
        <style>${`
          #scope_pfgf892 {
            & > div {
        ...
        <div class=${{invisible:m!=="open"||b[0]===c}}>
        ...
        <div class=${{invisible:m==="closed"}}>
          ...
          <div class='menu-label' onclick=${a}>Close</div>
        </div>
        ...
        display: flex;
        flex-direction: column;
      `;return`& { ${CSS.supports("position-anchor","--test")?t:r} ${n} }`},anchorRef$:()=>i.app$()?.ref}));return this.h`<a-menu props=${i} />`});d("toolbarAppLauncher",function(){let e=S(localStorage),s=V("hardcoded_newAppIdsObj"),o=Q(this.props.appIndex),i=C(),t=K(()=>({id:this.props.appId,key:this.props.appKey,workspaceKey:e.session_openWorkspaceKeys$()[0],index:o(),visibility:e[`session_appByKey_${this.props.appKey}_visibility$`](),icon:e[`session_appByKey_${this.props.appKey}_icon$`](),isNew:!!s()[this.props.appId],ref:i()})),{toggleMenu:r,app$:n}=A("<a-menu>"),a=()=>r({...t()}),p=K(()=>n().key===t().key?"--app-launchers-menu":"none"),l=K(()=>{switch(t().visibility){case"open":return f.colors.bg3Primary;case"minimized":return f.colors.bg3Secondary;case"closed":default:return f.colors.bg3}});return this.h`<div
    ref=${i}
    onclick=${a}
  ...
  </div>`});async function pe(e,s,o){if(!e||!s||o==null)throw new Error("appId, name and eKind are required");if(o===-1)return z("get",[[e,s,-1]],"permissions").then(p=>!!p.result);let i=IDBKeyRange.bound([e,s,-1],[e,s,o]),t=Promise.withResolvers();z("openKeyCursor",[i],"permissions",null,{p:t});let r,n,a=[e,s,o];for(;r=(await t.promise).result;){if(n=r.primaryKey[2],n===-1||n===o)return!0;Object.assign(t,Promise.withResolvers()),r.continue(a)}return!1}async function le(e,s,o){if(!e||!s||o==null)throw new Error("appId, name and eKind are required");return z("put",[{appId:e,name:s,eKind:o}],"permissions")}d("iconX",function(){let e=$({path$:["M18 6l-12 12","M6 6l12 12"],viewBox$:"2 2 20 20"});return this.h`<a-svg
    props=${{...e,...this.props}}
  />`});d("permissionDialog",function(){let e=P("<permission-dialog>",()=>({isOpen$(){return this.queue$().length>0},close(){let o=this.queue$().length,i=Promise.resolve();for(;o-- >0;)i=i.then(()=>this.resolveCurrent(!1))},queue$:[],getPermissionId(o){return`${o.app.id}:${o.name}:${o.eKind??""}`},isSingularPermission(o){return o.eKind==null||o.eKind===5||o.eKind===62},addPermissionRequest(o){this.queue$(i=>{let t;return this.isSingularPermission(o)&&(t=i.find(r=>r.id===o.id))?(t.promise.then(o.resolve).catch(o.reject),i):(i.push({id:o.id,app:{id:o.app.id,napp:o.app.napp,alias:o.app.alias,name:o.app.name,icon:{fx:o.app.icon?.fx,url:o.app.icon?.url}},name:o.name,eKind:o.eKind,meta:{...o.meta},promise:o.promise,resolve:o.resolve,reject:o.reject}),i)})},removeCurrent(o){if(this.queue$().length===0)return;let i=o??this.queue$()[0];this.queue$(t=>o?t.filter(r=>r.id!==i.id):t.slice(1))},async resolveCurrent(o,i){if(this.queue$().length===0)return;let t=i??this.queue$()[0];if(o){if(this.isSingularPermission(t)){t.resolve(!0),this.removeCurrent(i);return}await le(t.app.id,t.name,t.eKind),t.resolve(!0),this.removeCurrent(i)}else t.reject(new Error("Permission denied")),this.removeCurrent(i)},async queryPermission(o){return o.eKind==null||o.app.id&&o.name==="openApp"?!1:pe(o.app.id,o.name,o.eKind)},async requestPermission(o){if(await this.queryPermission(o))return!0;let t=Promise.withResolvers();return this.addPermissionRequest({...o,...t,id:this.getPermissionId(o)}),t.promise}})),s=$(()=>({isOpen$:e.isOpen$,close:e.close.bind(e),shouldAlwaysDisplay$:!0,render:R(function(){return this.h`<permission-dialog-stack />`})}));return this.h`<a-modal props=${s} />`});d("permissionDialogStack",function(){let e=S(localStorage),s=P("<permission-dialog>"),o=A("<permission-dialog-stack>",()=>({resolveCurrent:s.resolveCurrent.bind(s),eKindToText:{0:"profiles",1:"short text notes",3:"follow lists",4:"(legacy) direct messages",6:"short text renotes",7:"reactions",13:"message seals",14:"(public) chat messages",15:"(public) file decryption keys",16:"renotes",20:"pictures",21:"videos",22:"short vertical videos",1018:"poll responses",1059:"recipient directions",1068:"polls",1111:"comments",1222:"short voice notes",1244:"short voice comments",1984:"misconduct reports",7376:"nutzap redemption logs",9321:"nutzaps",9734:"bitcoin pre-payment data",9735:"bitcoin receipts",10002:"home server configurations",10019:"nutzap receiving addresses",27235:"API authentication requests",30008:"profile badges",30009:"profile badge definitions",30023:"long text notes",30311:"livestreams",30402:"classified listings",30403:"(draft) classified listings",31922:"date events",31923:"time events",31924:"calendars",31925:"event RSVPs",34600:"files",37348:"napp stalls",37349:"(next) napp stalls",37350:"(draft) napp stalls",37448:"napp bundles",37449:"(next) napp bundles",37450:"(draft) napp bundles"},getEKindToText(i,t){if(t==="readProfile")return"your profile";let r=this.eKindToText[i];return r||(i==null?r="an item":r=`kind ${i} items`),r},nameToText:{readProfile:"read",signEvent:"publish",encrypt:"encrypt",decrypt:"decrypt",openApp:"open"},getNameToText(i){return this.nameToText[i]||i},getPemissionText(i,t,r){let n;if(t===22242)n="access content that needs login";else if(t===5){let a=r?.params?.[0];if(!a)throw new Error("Missing event parameter for eKind 5 permission");let p=["e","a"],l=a.tags.filter(y=>p.includes(y[0])).length||1;n=`delete ${l} ${l===1?"item":"items"}`}else if(t===62){let a=event.tags.filter(l=>l[0]==="relay"),p=a.some(l=>l==="ALL_RELAYS")?1/0:a.length||1;n=`delete ALL your items from ${p===1/0?"ALL servers":`${p} ${p===1?"server":"servers"}`}`}else if(i==="openApp"){let{targetApp:a}=r??{};if(!a)throw new Error("Missing app parameter for openApp permission");let{[`session_appById_${a.id}_name$`]:p}=e,l=a.name||p()||a.alias||a.napp;if(l==null)throw new Error("Missing app name for openApp permission");n=`${this.getNameToText(i)} the ${l} napp`}if(!n){let a=this.getEKindToText(t,i);n=[this.getNameToText(i),a].filter(Boolean).join(" ")}return"Can I "+n+"?"},permissionRequests$(){return s.queue$()}}));return this.h`
    <style>${`
    ...
    <div id='permission-dialog-stack' class=${E.defaultTheme}>
      ${o.permissionRequests$().map((i,t)=>this.h({key:i.id})`
        <f-to-signals
          key=${i.id}
          props=${{from:["req","index"],req:i,index:t,render({req$:r,index$:n}){return this.h`<permission-dialog-card
                props=${{req$:r,index$:n}}
              />`}}}
        />
      `)}
    </div>
  `});d("permissionDialogCard",function(){let e=S(localStorage),s=A("<permission-dialog-stack>"),o=$(()=>({req$:this.props.req$,index$:this.props.index$,resolveCurrent(t){return s.resolveCurrent(t,this.req$())},isButtonsDisabled$:!1,allow(){return this.isButtonsDisabled$(!0),this.resolveCurrent(!0)},deny(){return this.isButtonsDisabled$(!0),this.resolveCurrent(!1)},permissionText$(){let t=this.req$();return s.getPemissionText(t.name,t.eKind,t.meta)},appName$(){let t=this.req$(),{[`session_appById_${t.app.id}_name$`]:r}=e,n=r();return t.app.name||n||t.app.alias||t.app.napp||"App"}})),i=$(()=>({app$:()=>({id:o.req$().app.id,index:"?"})}));return this.h`
    <style>${`
  ...
  `});d("icon-help-hexagon-filled",function(){let e=$({path$:["M10.425 1.414a3.33 3.33 0 0 1 3.026 -.097l.19 .097l6.775 3.995l.096 .063l.092 .077l.107 .075a3.224 3.224 0 0 1 1.266 2.188l.018 .202l.005 .204v7.284c0 1.106 -.57 2.129 -1.454 2.693l-.17 .1l-6.803 4.302c-.918 .504 -2.019 .535 -3.004 .068l-.196 -.1l-6.695 -4.237a3.225 3.225 0 0 1 -1.671 -2.619l-.007 -.207v-7.285c0 -1.106 .57 -2.128 1.476 -2.705l6.95 -4.098zm1.575 13.586a1 1 0 0 0 -.993 .883l-.007 .117l.007 .127a1 1 0 0 0 1.986 0l.007 -.117l-.007 -.127a1 1 0 0 0 -.993 -.883zm1.368 -6.673a2.98 2.98 0 0 0 -3.631 .728a1 1 0 0 0 1.44 1.383l.171 -.18a.98 .98 0 0 1 1.11 -.15a1 1 0 0 1 -.34 1.886l-.232 .012a1 1 0 0 0 .111 1.994a3 3 0 0 0 1.371 -5.673z"],viewBox$:"2 2 20 20",weight$:"fill",style$:()=>`
        path { stroke: none; }
        ${this.props.style$?.()||this.props.style||""}
      `});return this.h`<a-svg
    props=${{...e,...this.props}}
  />`});var ce="Confirmation",de="Are you sure?",ue="Yes",be=()=>new Error("Confirmation superseded"),we=e=>(e=new Error("Confirmation denied"))&&(e.code="DENIED_BY_USER")&&e,xe=e=>(e=new Error("Confirmation dialog closed"))&&(e.code="DENIED_BY_USER")&&e;d("confirmation-dialog",function(){let e=P("<confirmation-dialog>",()=>({currentRequest$:null,lastRequest$:null,isOpen$(){return!!this.currentRequest$()},title$(){return(this.currentRequest$()??this.lastRequest$())?.title??ce},message$(){return(this.currentRequest$()??this.lastRequest$())?.message??de},confirmText$(){return(this.currentRequest$()??this.lastRequest$())?.confirmText??ue},resolveCurrent(){let o=this.currentRequest$();o&&(this.lastRequest$(o),this.currentRequest$(null),o.resolve(!0))},rejectCurrent(o=we()){let i=this.currentRequest$();i&&(this.lastRequest$(i),this.currentRequest$(null),i.reject(o))},close(){this.rejectCurrent(xe())},requestConfirmation({title:o=ce,message:i=de,confirmText:t=ue}={}){let r=this.currentRequest$();r&&r.reject(be());let{promise:n,resolve:a,reject:p}=Promise.withResolvers();return this.currentRequest$({title:o,message:i,confirmText:t,resolve:a,reject:p}),n}})),s=$(()=>({isOpen$:e.isOpen$,close:e.close.bind(e),shouldAlwaysDisplay$:!0,render:R(function(){return this.h`<confirmation-dialog-card />`})}));return this.h`<a-modal props=${s} />`});d("confirmation-dialog-card",function(){let e=P("<confirmation-dialog>"),s=$(()=>({isButtonsDisabled$:!1,title$:e.title$,message$:e.message$,confirmText$:e.confirmText$,confirm(){this.isButtonsDisabled$()||(this.isButtonsDisabled$(!0),e.resolveCurrent())},deny(){this.isButtonsDisabled$()||(this.isButtonsDisabled$(!0),e.rejectCurrent())}}));return _(({track:o})=>{o(()=>e.currentRequest$()),s.isButtonsDisabled$(!1)}),this.h`
    <style>${`
  ...
  `});d("multi-napp",function(){return _(()=>{W.initCleanupJob(),W.initUpdateCheckJob()}),M(U),L(()=>({isOpen$:!1,open(){this.isOpen$(!0)},close(){this.isOpen$(!1)}})),this.h`
    <vault-modal />
    <permission-dialog />
    <confirmation-dialog />
    <a-screen />
