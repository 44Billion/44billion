...
describe('nostrdb', () => {
  ...
  it('searches strictly validated personal-copy wrappers through decrypted inner JSON', async () => {
    ...
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const matchInner = personalCopyRumor({ content: 'secret whisper' })
    const missInner = personalCopyRumor({ pubkey: B, content: 'public chatter' })
    const match = await personalCopyWrapper({
      id: hexId(30003),
      owner,
      inner: matchInner,
      context: 'search',
    ...
    const miss = await personalCopyWrapper({
      id: hexId(30004),
      owner,
      inner: missInner,
      context: 'search',
    ...
    rememberPersonalCopy(plaintexts, match, matchInner)
    rememberPersonalCopy(plaintexts, miss, missInner)
    ...
    assert.deepEqual(
      (await queryResults(db, {
        kinds: [eventKinds.PERSONAL_COPY],
        '#k': ['1'],
        search: 'whisper'
      })).map(event => event.id),
      [match.id]
  ...
  it('regenerates derived mirror tags while preserving context metadata', async () => {
    const owner = hexId(30005)
    const plaintexts = new Map()
    const inner = personalCopyRumor({ tags: [['t', 'topicexample']] })
    ...
    wrapper.tags[wrapper.tags.findIndex(tag => tag[0] === 'o')] = ['o', 'stale']
    ...
    const result = await db.add(wrapper, {
      signEvent: eventTemplate => {
        template = cloneJson(eventTemplate)
        return signedFromTemplate(eventTemplate, { id: hexId(30007), pubkey: owner })
      }
    })

    assertAddOk(result, { code: 'stored', stored: true, published: true })
    assert.deepEqual(template.tags.slice(0, 3), [
      ['k', '1'],
      ['c', 'obf:1006::dm'],
      ['v', PERSONAL_COPY_PROVENANCE.DIRECT_RUMOR]
    ])
    assert.equal(template.tags.some(tag => tag[1] === 'obf:1006:#t:topicexample'), true)
    assert.equal(template.tags.some(tag => tag[1] === 'stale'), false)
  })

  it('accepts every strict inner shape with its matching provenance', async () => {
    const selfSigned = signedPersonalCopyInner(5, { content: 'signed self' })
    const owner = selfSigned.pubkey
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const values = [
      {
        inner: personalCopyTemplate({ content: 'template' }),
        provenance: PERSONAL_COPY_PROVENANCE.DIRECT_RUMOR
      },
      {
        inner: personalCopyRumor({ content: 'direct third party' }),
        provenance: PERSONAL_COPY_PROVENANCE.DIRECT_RUMOR
      },
      {
        inner: personalCopyRumor({ pubkey: B, content: 'hearsay third party' }),
    ...
    for (let index = 0; index < values.length; index++) {
      ...
      const wrapper = await personalCopyWrapper({
        id: hexId(30010 + index),
        owner,
        inner: value.inner,
        context: `valid-${index}`,
        content: `cipher-valid-${index}`,
        provenance: value.provenance
      })
      rememberPersonalCopy(plaintexts, wrapper, value.inner)
    ...
    assert.deepEqual(
      new Set(stored.map(event => event.tags.find(tag => tag[0] === 'v')[1])),
      new Set(Object.values(PERSONAL_COPY_PROVENANCE))
    )
  })

  it('rejects malformed metadata, retired hearsay, timestamp drift, and invalid inner fields', async () => {
    const owner = hexId(30020)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const inner = personalCopyRumor()
    const base = await personalCopyWrapper({
      id: hexId(30021),
      owner,
      inner,
      context: 'strict',
    ...
    const cases = [
      wrapper => { wrapper.tags = wrapper.tags.filter(tag => tag[0] !== 'k') },
      wrapper => { wrapper.tags.push(['k', '1']) },
      wrapper => { wrapper.tags.find(tag => tag[0] === 'k')[1] = '2' },
      wrapper => { wrapper.tags = wrapper.tags.filter(tag => tag[0] !== 'c') },
      wrapper => { wrapper.tags.push(['c', 'obf:1006::strict']) },
      wrapper => { wrapper.tags.find(tag => tag[0] === 'c').push('extra') },
      wrapper => { wrapper.tags = wrapper.tags.filter(tag => tag[0] !== 'v') },
      wrapper => { wrapper.tags.find(tag => tag[0] === 'v')[1] = '9' },
      wrapper => { wrapper.tags.find(tag => tag[0] === 'v')[1] = PERSONAL_COPY_PROVENANCE.SIGNED_EVENT },
      wrapper => { wrapper.tags.push(['v', PERSONAL_COPY_PROVENANCE.DIRECT_RUMOR]) },
      wrapper => { wrapper.tags.push(['hearsay']) },
      wrapper => { wrapper.created_at-- },
      wrapper => { wrapper.created_at++ },
      (wrapper, value) => { value.inner = { ...inner, unexpected: true } },
      (wrapper, value) => { value.inner = { ...inner, id: hexId(998) } },
      (wrapper, value) => { value.inner = invalidSigned }
    ]

    for (let index = 0; index < cases.length; index++) {
      const wrapper = cloneJson(base)
      wrapper.id = hexId(30030 + index)
      wrapper.content = `cipher-invalid-${index}`
      const value = { inner }
      cases[index](wrapper, value)
      rememberPersonalCopy(plaintexts, wrapper, value.inner)
      assertAddNotOk(await db.add(wrapper), { code: 'invalid' })
    }

    const internalInvalid = cloneJson(base)
    internalInvalid.id = hexId(30090)
    internalInvalid.tags = internalInvalid.tags.filter(tag => tag[0] !== 'v')
    rememberPersonalCopy(plaintexts, internalInvalid, inner)
    assertAddNotOk(await db.addEvent(internalInvalid), { code: 'invalid' })

    assert.deepEqual(await queryResults(db, { kinds: [eventKinds.PERSONAL_COPY] }), [])
  })

  it('uses the same source identity for a self-owned template and its signed event', async () => {
    for (let order = 0; order < 2; order++) {
      const signed = signedPersonalCopyInner(10 + order, {
        created_at: 50,
        tags: [['d', 'draft']],
        content: 'same source'
      })
      const owner = signed.pubkey
      const template = personalCopyTemplate({
        kind: signed.kind,
        created_at: signed.created_at,
        tags: signed.tags,
        content: signed.content
      })
      assert.equal(personalCopySourceId(template, { wrapperPubkey: owner }), signed.id)

      const plaintexts = new Map()
      const db = personalCopyDb(owner, plaintexts)
      const direct = await personalCopyWrapper({
        id: hexId(30100 + order * 10),
        owner,
        inner: template,
        context: 'draft',
        content: `cipher-template-${order}`
      })
      const verified = await personalCopyWrapper({
        id: hexId(30101 + order * 10),
        owner,
        inner: signed,
        context: 'draft',
        content: `cipher-signed-${order}`,
        provenance: PERSONAL_COPY_PROVENANCE.SIGNED_EVENT
      })
      rememberPersonalCopy(plaintexts, direct, template)
      rememberPersonalCopy(plaintexts, verified, signed)

      const ordered = order === 0 ? [direct, verified] : [verified, direct]
      assertAddOk(await db.add(ordered[0]))
      assertAddOk(await db.add(ordered[1]))

      const [stored] = await queryResults(db, { kinds: [eventKinds.PERSONAL_COPY] })
      assert.equal(stored.id, verified.id)
      assert.equal(stored.tags.find(tag => tag[0] === 'v')[1], PERSONAL_COPY_PROVENANCE.SIGNED_EVENT)
    }
  })

  it('prefers a verified third-party event over direct and hearsay rumors', async () => {
    const owner = hexId(30119)
    const signed = signedPersonalCopyInner(20, {
      created_at: 55,
      content: 'verified third party'
    })
    const rumor = {
    ...
    const hearsay = await personalCopyWrapper({
      ...
      inner: rumor,
      context: 'verified',
      content: 'cipher-verified-hearsay',
      provenance: PERSONAL_COPY_PROVENANCE.HEARSAY_RUMOR
    })
    const direct = await personalCopyWrapper({
      id: hexId(30117),
      ...
      inner: rumor,
      context: 'verified',
      content: 'cipher-verified-direct'
    })
    const verified = await personalCopyWrapper({
      id: hexId(30118),
      owner,
      inner: signed,
      context: 'verified',
      content: 'cipher-verified-event',
      provenance: PERSONAL_COPY_PROVENANCE.SIGNED_EVENT
    })
    rememberPersonalCopy(plaintexts, hearsay, rumor)
    rememberPersonalCopy(plaintexts, direct, rumor)
    rememberPersonalCopy(plaintexts, verified, signed)

    assertAddOk(await db.add(hearsay))
    assertAddOk(await db.add(direct))
    assertAddOk(await db.add(verified))
    assert.deepEqual(
      (await queryResults(db, { kinds: [eventKinds.PERSONAL_COPY] })).map(event => event.id),
      [verified.id]
    )
  })

  it('prefers direct rumors over hearsay in either arrival order', async () => {
    for (let order = 0; order < 2; order++) {
      const owner = hexId(30120 + order)
      const plaintexts = new Map()
      const db = personalCopyDb(owner, plaintexts)
      const inner = personalCopyRumor({ created_at: 60 })
      const direct = await personalCopyWrapper({
        id: hexId(30130 + order * 10),
        owner,
        inner,
        context: 'conversation',
        content: `cipher-direct-${order}`
      })
      const hearsay = await personalCopyWrapper({
        id: hexId(30131 + order * 10),
        owner,
        inner,
        context: 'conversation',
        content: `cipher-hearsay-${order}`,
        provenance: PERSONAL_COPY_PROVENANCE.HEARSAY_RUMOR
      })
      rememberPersonalCopy(plaintexts, direct, inner)
      rememberPersonalCopy(plaintexts, hearsay, inner)

      const ordered = order === 0 ? [hearsay, direct] : [direct, hearsay]
      assertAddOk(await db.add(ordered[0]))
      assertAddOk(await db.add(ordered[1]))
  ...
  it('keeps the lower outer ID when source, context, provenance, and timestamp tie', async () => {
    const owner = hexId(30150)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const inner = personalCopyRumor({ created_at: 70 })
    const higher = await personalCopyWrapper({
      id: hexId(9999),
      owner,
      inner,
      context: 'tie',
      content: 'cipher-higher'
    })
    const lower = await personalCopyWrapper({
      id: hexId(1),
      owner,
      inner,
      context: 'tie',
      content: 'cipher-lower'
    })
    rememberPersonalCopy(plaintexts, higher, inner)
    rememberPersonalCopy(plaintexts, lower, inner)

    assertAddOk(await db.add(higher))
    assertAddOk(await db.add(lower))
  ...
  it('keeps matching source IDs independent across contexts', async () => {
    const owner = hexId(30160)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const inner = personalCopyRumor({ created_at: 80 })
    const first = await personalCopyWrapper({
      id: hexId(30161),
      owner,
      inner,
      context: 'first',
      content: 'cipher-first'
    })
    const second = await personalCopyWrapper({
      ...
      inner,
      context: 'second',
      content: 'cipher-second',
      provenance: PERSONAL_COPY_PROVENANCE.HEARSAY_RUMOR
    })
    rememberPersonalCopy(plaintexts, first, inner)
    rememberPersonalCopy(plaintexts, second, inner)

    assertAddOk(await db.add(first))
    assertAddOk(await db.add(second))

    const stored = await queryResults(db, { kinds: [eventKinds.PERSONAL_COPY] })
    assert.deepEqual(new Set(stored.map(event => event.id)), new Set([first.id, second.id]))
  })

  it('reconciles more than the public query limit within one context', async () => {
    const owner = hexId(30170)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const inner = personalCopyRumor({ created_at: 90 })
    await openNostrDb(owner)

    for (let index = 0; index < 205; index++) {
      const wrapper = await personalCopyWrapper({
        id: hexId(30200 + index),
        owner,
        inner,
        context: 'bulk',
        content: `cipher-bulk-${index}`,
        provenance: PERSONAL_COPY_PROVENANCE.HEARSAY_RUMOR
      })
      rememberPersonalCopy(plaintexts, wrapper, inner)
      seedPersonalCopyRecord(owner, wrapper)
    }
  ...
  it('retries once when the same-context candidate snapshot changes before commit', async () => {
    const owner = hexId(30510)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const inner = personalCopyRumor({ created_at: 100 })
    const first = await personalCopyWrapper({
      id: hexId(30511),
      owner,
      inner,
      context: 'retry',
      content: 'cipher-retry-first',
      provenance: PERSONAL_COPY_PROVENANCE.HEARSAY_RUMOR
    })
    const concurrent = await personalCopyWrapper({
      ...
      inner,
      context: 'retry',
      content: 'cipher-retry-concurrent',
      provenance: PERSONAL_COPY_PROVENANCE.HEARSAY_RUMOR
    })
    const direct = await personalCopyWrapper({
      id: hexId(30513),
      ...
      inner,
      context: 'retry',
      content: 'cipher-retry-direct'
    })
    for (const wrapper of [first, concurrent, direct]) rememberPersonalCopy(plaintexts, wrapper, inner)
    assertAddOk(await db.add(first))

    const prepare = db.preparePersonalCopyProvenance
    let calls = 0
    db.preparePersonalCopyProvenance = async function (...args) {
      const resolution = await prepare.call(this, ...args)
      calls++
      if (calls === 1) seedPersonalCopyRecord(owner, concurrent)
      return resolution
    }
  ...
  it('does not delete an existing winner when derived-tag signing fails', async () => {
    const owner = hexId(30520)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const inner = personalCopyRumor({ created_at: 110 })
    const hearsay = await personalCopyWrapper({
      id: hexId(30521),
      owner,
      inner,
      context: 'atomic',
      content: 'cipher-atomic-hearsay',
      provenance: PERSONAL_COPY_PROVENANCE.HEARSAY_RUMOR
    })
    const direct = await personalCopyWrapper({
      id: hexId(30522),
      ...
      inner,
      context: 'atomic',
      content: 'cipher-atomic-direct'
    })
    direct.tags[direct.tags.findIndex(tag => tag[0] === 'o')] = ['o', 'stale']
    rememberPersonalCopy(plaintexts, hearsay, inner)
    rememberPersonalCopy(plaintexts, direct, inner)
    assertAddOk(await db.add(hearsay))

    assertAddNotOk(await db.add(direct, {
  ...
  it('replaces an older non-owner copy at the same derived address without CRDT merging', async () => {
    ...
    rememberPersonalCopy(plaintexts, stale, staleInner)
    let signCalls = 0
    const signEvent = async () => {
      signCalls++
      throw new Error('personal copies must not be CRDT merged')
    }

    assertAddOk(await db.add(older, { mergeReplaceable: true, signEvent }))
    assertAddOk(await db.add(newer, { mergeReplaceable: true, signEvent }), {
      code: 'replaced',
      stored: true,
      published: true
    })
    assertAddOk(await db.addEvent(stale, { forceCoordinateReplace: true }), {
      code: 'superseded',
      stored: false,
      published: false
    })
  ...
  it('ignores hearsay private deletion envelopes without applying them', async () => {
    const owner = hexId(30740)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const targetInner = personalCopyTemplate({ kind: 9, created_at: 100, content: 'stays' })
    const envelopeInner = personalCopyTemplate({
      kind: 5,
      created_at: 200,
      tags: [['e', personalCopySourceId(targetInner, { wrapperPubkey: owner })], ['k', '9']],
      content: ''
    })
    const rumor = { ...envelopeInner, pubkey: B }
    const target = await personalCopyWrapper({ id: hexId(30741), owner, inner: targetInner, context: 'dm:hearsay', content: 'cipher-target' })
    ...
    rememberPersonalCopy(plaintexts, envelope, rumor)

    assertAddOk(await db.add(target))

    for (const add of [
      () => db.add(envelope),
      () => db.add(envelope, { mergeSource: 'sync' }),
      () => db.addEvent(envelope)
    ]) {
    ...
    assert.deepEqual((await queryResults(db, { ids: [target.id] })).map(event => event.id), [target.id])
    assert.deepEqual(await queryResults(db, { ids: [envelope.id] }), [])
  })

  it('does not apply a superseded private deletion envelope', async () => {
    const owner = hexId(30745)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    ...
    const target = await personalCopyWrapper({ id: hexId(30746), owner, inner: targetInner, context: 'dm:supersede', content: 'cipher-target' })
    const winner = await personalCopyWrapper({ id: hexId(30747), owner, inner: envelopeInner, context: 'dm:supersede', content: 'cipher-envelope-winner' })
    const loser = await personalCopyWrapper({ id: hexId(30748), owner, inner: envelopeInner, context: 'dm:supersede', content: 'cipher-envelope-loser' })
    for (const [wrapper, inner] of [[target, targetInner], [winner, envelopeInner], [loser, envelopeInner]]) {
      rememberPersonalCopy(plaintexts, wrapper, inner)
    }

    assertAddOk(await db.add(target))
    assertAddOk(await db.add(winner))
    assert.deepEqual(await queryResults(db, { ids: [target.id] }), [])

    const result = await db.add(loser)
    assert.equal(result.code, 'superseded')
    assert.equal(result.stored, false)
    assert.deepEqual(await queryResults(db, { ids: [loser.id] }), [])

    const tombstone = fakeStore(owner, 'deletions').records.get(deletionEventRef(target.id, owner))
  ...
  it('compacts private deletion envelopes per context and rekeys pending markers', async () => {
    ...
    const plaintexts = new Map()
    const db = personalCopyMutableDb(owner, plaintexts)
    const context = 'dm:compact'
    const firstTargetInner = personalCopyTemplate({ kind: 9, created_at: 10, content: 'first' })
    const secondTargetInner = personalCopyTemplate({ kind: 9, created_at: 11, content: 'second' })
    const firstTarget = await personalCopyWrapper({ id: hexId(30751), owner, inner: firstTargetInner, context, content: 'cipher-target-1' })
    const secondTarget = await personalCopyWrapper({ id: hexId(30752), owner, inner: secondTargetInner, context, content: 'cipher-target-2' })
    const firstInner = personalCopyTemplate({
      kind: 5,
      created_at: 100,
      tags: [['e', personalCopySourceId(firstTargetInner, { wrapperPubkey: owner })], ['k', '9']],
      content: ''
    })
    const secondInner = personalCopyTemplate({
      kind: 5,
      created_at: 101,
      tags: [['e', personalCopySourceId(secondTargetInner, { wrapperPubkey: owner })], ['k', '9']],
      content: ''
    })
    const firstEnvelope = await personalCopyWrapper({ id: hexId(30753), owner, inner: firstInner, context, content: 'cipher-envelope-1' })
    const secondEnvelope = await personalCopyWrapper({ id: hexId(30754), owner, inner: secondInner, context, content: 'cipher-envelope-2' })
    for (const [wrapper, inner] of [[firstTarget, firstTargetInner], [secondTarget, secondTargetInner], [firstEnvelope, firstInner], [secondEnvelope, secondInner]]) {
      rememberPersonalCopy(plaintexts, wrapper, inner)
    }

    assertAddOk(await db.add(firstTarget))
    assertAddOk(await db.add(secondTarget))
    assertAddOk(await db.add(firstEnvelope))
    assertAddOk(await db.add(secondEnvelope))

    const compactedId = hexId(30755)
    const result = await db.compactDeletionRequests({
      author: owner,
      createdAt: 200,
      signEvent: template => signedFromTemplate(template, { id: compactedId, pubkey: owner })
    })
    const expectedTargets = [
      ['e', personalCopySourceId(firstTargetInner, { wrapperPubkey: owner })],
      ['e', personalCopySourceId(secondTargetInner, { wrapperPubkey: owner })],
      ['k', '9']
    ]

    assert.equal(result.compacted, true)
    assert.equal(result.created.id, compactedId)
    assert.deepEqual([...result.consumed].sort(), [firstEnvelope.id, secondEnvelope.id].sort())
    assert.deepEqual(
      result.targets.map(tag => tag.join(':')).sort(),
      expectedTargets.map(tag => tag.join(':')).sort()
    )
    assert.deepEqual(await queryResults(db, { ids: [firstEnvelope.id, secondEnvelope.id] }), [])

    const stored = await queryResults(db, { ids: [compactedId] })
    assert.deepEqual(stored.map(event => event.id), [compactedId])
    const compactedInner = JSON.parse(plaintexts.get(stored[0].content))
    assert.deepEqual(compactedInner.tags, result.targets)
    assert.equal(stored[0].tags.some(tag => tag[0] === 'o' && tag[1] === 'obf:1006:#k:9'), true)

    const pending = await db.pendingInnerMirrors(await openNostrDb(owner))
    const contextValue = await personalCopyObfuscate(context, 1006, '')
    assert.equal(pending.get(contextValue).has(`obf:1006:.id:${personalCopySourceId(firstTargetInner, { wrapperPubkey: owner })}`), true)
    assert.equal(pending.get(contextValue).has(`obf:1006:.id:${personalCopySourceId(secondTargetInner, { wrapperPubkey: owner })}`), true)
  })

  it('prunes private deletion envelopes and releases their pending markers', async () => {
    const owner = hexId(30760)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const context = 'dm:prune'
    const targetInner = personalCopyTemplate({ kind: 9, created_at: 10, content: 'target' })
    const lateInner = personalCopyTemplate({ kind: 9, created_at: 11, content: 'late' })
    const envelopeInner = personalCopyTemplate({
      kind: 5,
      created_at: 100,
      tags: [
        ['e', personalCopySourceId(targetInner, { wrapperPubkey: owner })],
        ['e', personalCopySourceId(lateInner, { wrapperPubkey: owner })],
        ['k', '9']
      ],
      content: ''
    })
    const target = await personalCopyWrapper({ id: hexId(30761), owner, inner: targetInner, context, content: 'cipher-target' })
    const late = await personalCopyWrapper({ id: hexId(30762), owner, inner: lateInner, context, content: 'cipher-late' })
    const envelope = await personalCopyWrapper({ id: hexId(30763), owner, inner: envelopeInner, context, content: 'cipher-envelope' })
    for (const [wrapper, inner] of [[target, targetInner], [late, lateInner], [envelope, envelopeInner]]) {
      rememberPersonalCopy(plaintexts, wrapper, inner)
    }
    ...
    assertAddOk(await db.addEvent(envelope, { now: 1 }))
    assert.deepEqual(await queryResults(db, { ids: [target.id] }), [])
    assertAddNotOk(await db.add(late), { code: 'blocked' })

    const raw = await openNostrDb(owner)
    const pending = await db.pendingInnerMirrors(raw)
    assert.equal(pending.get(await personalCopyObfuscate(context, 1006, '')).size, 2)
    ...
    assert.deepEqual(pruned.deleted, [envelope.id])
    assert.deepEqual(await queryResults(db, { ids: [envelope.id] }), [])
    assert.equal(raw.pendingInnerDeletions, null)

    assertAddOk(await db.add(target))
    assertAddOk(await db.add(late))
  })

  it('skips legacy hearsay envelopes during compaction and prunes them', async () => {
    const owner = hexId(30770)
    const plaintexts = new Map()
    const db = personalCopyMutableDb(owner, plaintexts)
    const context = 'dm:legacy-hearsay'
    const firstInner = { pubkey: B, kind: 5, created_at: 100, tags: [['e', hexId(30771)], ['k', '9']], content: '' }
    const secondInner = { ...firstInner, created_at: 101, tags: [['e', hexId(30772)], ['k', '9']] }
    const first = await personalCopyWrapper({
      id: hexId(30773),
      owner,
      inner: firstInner,
      context,
      content: 'cipher-legacy-1',
      provenance: PERSONAL_COPY_PROVENANCE.HEARSAY_RUMOR
    })
    const second = await personalCopyWrapper({
      id: hexId(30774),
      ...
      inner: secondInner,
      context,
      content: 'cipher-legacy-2',
      provenance: PERSONAL_COPY_PROVENANCE.HEARSAY_RUMOR
    })
    rememberPersonalCopy(plaintexts, first, firstInner)
    rememberPersonalCopy(plaintexts, second, secondInner)
    await openNostrDb(owner)
    seedPersonalCopyRecord(owner, first)
    seedPersonalCopyRecord(owner, second)
    for (const wrapper of [first, second]) {
      fakeStore(owner, 'events').records.get(eventIdIndexKey(wrapper.id)).ra = 1000
    }

    const compacted = await db.compactDeletionRequests({
      author: owner,
      createdAt: 200,
      signEvent: template => signedFromTemplate(template, { id: hexId(30775), pubkey: owner })
    })
  ...
  it('keeps private deletion compaction scoped to one context', async () => {
    const owner = hexId(30780)
    const plaintexts = new Map()
    const db = personalCopyMutableDb(owner, plaintexts)
    const makeEnvelope = async (context, createdAt, targetId, wrapperId) => {
      const inner = personalCopyTemplate({
        kind: 5,
        created_at: createdAt,
        tags: [['e', hexId(targetId)], ['k', '9']],
      ...
      const wrapper = await personalCopyWrapper({
        ...
        inner,
        context,
        content: `cipher-${wrapperId}`
      })
      rememberPersonalCopy(plaintexts, wrapper, inner)
      return wrapper
    }
    const firstA = await makeEnvelope('dm:compact-a', 100, 30801, 30781)
    const secondA = await makeEnvelope('dm:compact-a', 101, 30802, 30782)
    const firstB = await makeEnvelope('dm:compact-b', 110, 30803, 30783)
    const secondB = await makeEnvelope('dm:compact-b', 111, 30804, 30784)
    for (const wrapper of [firstA, secondA, firstB, secondB]) {
      assertAddOk(await db.addEvent(wrapper, { now: 1 }))
    }

    const firstResult = await db.compactDeletionRequests({
      author: owner,
      createdAt: 200,
      signEvent: template => signedFromTemplate(template, { id: hexId(30790), pubkey: owner })
    })
    assert.deepEqual([...firstResult.consumed].sort(), [firstA.id, secondA.id].sort())
    assert.deepEqual((await queryResults(db, { ids: [firstB.id, secondB.id] })).map(event => event.id).sort(), [firstB.id, secondB.id].sort())

    const secondResult = await db.compactDeletionRequests({
      author: owner,
      createdAt: 201,
      signEvent: template => signedFromTemplate(template, { id: hexId(30791), pubkey: owner })
    })
    assert.deepEqual([...secondResult.consumed].sort(), [firstB.id, secondB.id].sort())
  })

  it('skips private deletion compaction without vault encryption', async () => {
    const owner = hexId(30810)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const buildEnvelope = async (createdAt, targetId, wrapperId) => {
      const inner = personalCopyTemplate({
        kind: 5,
        created_at: createdAt,
        tags: [['e', hexId(targetId)], ['k', '9']],
      ...
      const wrapper = await personalCopyWrapper({
        ...
        owner,
        inner,
        context: 'dm:no-encrypt',
        content: `cipher-no-encrypt-${wrapperId}`
      })
      rememberPersonalCopy(plaintexts, wrapper, inner)
      return wrapper
    }
    const first = await buildEnvelope(100, 30811, 30812)
    const second = await buildEnvelope(101, 30813, 30814)
    assertAddOk(await db.addEvent(first, { now: 1 }))
    assertAddOk(await db.addEvent(second, { now: 1 }))

    const result = await db.compactDeletionRequests({
      author: owner,
      createdAt: 200,
      signEvent: template => signedFromTemplate(template, { id: hexId(30815), pubkey: owner })
    })
    assert.equal(result.compacted, false)
    assert.deepEqual((await queryResults(db, { ids: [first.id, second.id] })).map(event => event.id).sort(), [first.id, second.id].sort())
  })

  it('counts public and private deletion requests together when pruning', async () => {
    const owner = hexId(30800)
    const plaintexts = new Map()
    const db = personalCopyDb(owner, plaintexts)
    const envelopeInner = personalCopyTemplate({
    ...
    const envelope = await personalCopyWrapper({
      ...
      owner,
      inner: envelopeInner,
      context: 'dm:mixed-prune',
      content: 'cipher-mixed-envelope'
    })
    const publicDeletion = event({
      id: hexId(30803),
      pubkey: owner,
      kind: 5,
      created_at: 101,
      tags: [['e', hexId(30804)], ['e', hexId(30805)], ['e', hexId(30806)]]
    })
    rememberPersonalCopy(plaintexts, envelope, envelopeInner)
    assertAddOk(await db.addEvent(envelope, { now: 1 }))
    assertAddOk(await db.addEvent(publicDeletion, { now: 1 }))
    ...
    assert.deepEqual(pruned.deleted, [envelope.id])
    assert.deepEqual((await queryResults(db, { ids: [publicDeletion.id] })).map(event => event.id), [publicDeletion.id])
    assert.deepEqual(await queryResults(db, { ids: [envelope.id] }), [])
  })
...
async function personalCopyWrapper ({
  id,
  owner,
  inner,
  context = '',
...
function personalCopyRumor (options = {}) {
  return {
    pubkey: options.pubkey ?? A,
    kind: options.kind ?? 1,
    created_at: options.created_at ?? 1,
    tags: options.tags ?? [],
    content: options.content ?? ''
  }
}

function personalCopyTemplate (options = {}) {
  return {
    kind: options.kind ?? 1,
    created_at: options.created_at ?? 1,
    tags: options.tags ?? [],
...
function signedPersonalCopyInner (seed, options = {}) {
...
function personalCopyObfuscate (value, kind, scope) {
...
function personalCopyMutableDb (owner, plaintexts) {
  let serial = 0
  return getNostrDb(owner, {
    personalCopyDecrypt: async wrapper => plaintexts.get(wrapper.content) ?? '{}',
    personalCopyEncrypt: async (kind, plaintext) => {
      const content = `cipher-compacted-${++serial}-${kind}`
      plaintexts.set(content, plaintext)
      return content
    },
    personalCopyObfuscate
...
function rememberPersonalCopy (plaintexts, wrapper, inner) {
  plaintexts.set(wrapper.content, JSON.stringify(inner))
}

function seedPersonalCopyRecord (owner, wrapper) {
  const record = toStoredRecord(wrapper)
  fakeStore(owner, EVENTS_STORE).records.set(record.i, record)
}
