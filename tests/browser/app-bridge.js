...
try {
  ...
  const checkWidgetFit = async () => {
    await until("document.querySelector('widget-window iframe')?.style.width==='360px'", '2x2 widget uses its virtual viewport without manual resize')
    const sizes = await evaluate(`(()=>{
      const frame=document.querySelector('widget-window iframe');
      const root=frame.closest('.widget-window-root');
      return {viewport:frame.offsetWidth,displayed:frame.getBoundingClientRect().width,cell:root.getBoundingClientRect().width};
    })()`)
    assert.equal(sizes.viewport, 360)
    assert.ok(sizes.cell < sizes.viewport, 'fixture exercises the narrow-widget fit')
    assert.ok(Math.abs(sizes.displayed - sizes.cell) < 1, 'virtual viewport is scaled to the widget width')
  }
  await checkWidgetFit()
  const beforeReloadToken = await evaluate("fixture.loaded.find(item=>item.metadata.instanceKey==='widget').token")
  await cdp('Page.reload')
  await until(`window.fixture && ${loaded('widget')} && fixture.loaded.some(item=>item.metadata.instanceKey==='widget' && item.token!==${JSON.stringify(beforeReloadToken)})`, 'persisted 2x2 widget loads after launcher reload', 12000)
  await checkWidgetFit()
  const widgetToken = await evaluate("fixture.loaded.find(item=>item.metadata.instanceKey==='widget').token")
  const setWidgetMinWidth = width => evaluate(`document.querySelector('widget-window iframe').contentWindow.postMessage({code:'FIXTURE_MIN_WIDTH',width:${width}},'http://0.localhost:${port}')`)
  await setWidgetMinWidth(0)
  await until("document.querySelector('widget-window iframe').style.transform===''", 'setMinWidth(0) disables the virtual viewport')
  await setWidgetMinWidth(360)
  await checkWidgetFit()
  await evaluate('fixture.storage.local_widgets$(records=>({...records,widget:{...records.widget,desired:{w:7,h:2}}}))')
  await until("document.querySelector('widget-window iframe').style.transform===''", 'wide widget uses its actual width')
  await evaluate('fixture.storage.local_widgets$(records=>({...records,widget:{...records.widget,desired:{w:2,h:2}}}))')
  await checkWidgetFit()
  assert.equal(await evaluate("fixture.loaded.filter(item=>item.metadata.instanceKey==='widget').at(-1).token"), widgetToken, 'fitting does not reload the widget document')
  await evaluate("fixture.tabStorage.session_widgetByKey_widget_visibility$('closed')")
  await until(`!${loaded('widget')}`, 'widget closes before fit regression check')
  await evaluate("fixture.tabStorage.session_widgetByKey_widget_visibility$('open')")
  await until(loaded('widget'), 'widget reopens with the same geometry')
  await checkWidgetFit()
  await setVisibility('window', 'open')
  ...
  const disposedBridgeId = await evaluate(`${bridge}.bridgeId`)
  ...
  assert.notEqual(await evaluate(`${bridge}.bridgeId`), disposedBridgeId)
