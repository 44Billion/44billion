...
;(async () => {
  ...
  await select('tall')
  assert.equal(await evaluate(`${root('tall')}.querySelectorAll('.widget-remove-button').length`), 2)
  await evaluate(`${root('tall')}.querySelector('.widget-pin-button').click()`)
  await wait(120)
  assert.equal(await evaluate(`${root('tall')}.querySelector('.widget-pin-button').getAttribute('aria-pressed')`), 'true')
  ...
  await wait(100)
  const touchPoint = await evaluate(`(()=>{const r=${root('tall')}.getBoundingClientRect();return {x:r.x+30,y:r.y+40}})()`)
