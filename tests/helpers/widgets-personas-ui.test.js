...
describe('widgets and personas wiring', () => {
  it('exposes the persona APIs from the app page', async () => {
    ...
    assert.match(appPage, /startAutoFitIfWidget/)
    ...
    assert.match(appPage, /reportOverflow/)
    assert.match(appPage, /AUTO_FIT_WIDE_MIN_WIDTH/)
    assert.match(appPage, /isWidget/)
  })
  ...
  it('routes persona requests and scoped NIP07 calls through the app bridge', async () => {
    ...
    assert.match(appBridge, /onAutoFitOverflow/)
  })
