...
describe('widgets and personas wiring', () => {
  it('exposes the persona APIs from the app page', async () => {
    ...
    assert.match(appPage, /createWidgetDragClient/)
    assert.match(appPage, /widgetDragClient\.setSelectMode/)
    assert.match(appPage, /isWidget: \(\) => autoFitIsWidget/)
    assert.match(appPage, /sendDrag: sendWidgetDrag/)
  ...
  it('registers the widgets layer and Add Widget menu entry', async () => {
    ...
    assert.match(widgets, /widget-window-closed \.widget-clip/)
    assert.match(widgets, /visibility: hidden/)
    assert.doesNotMatch(widgets, /appIframeRef\$\(null\)/)
    assert.doesNotMatch(widgets, /icon-grip-vertical/)
    assert.match(widgets, /icon-pinned/)
    ...
    assert.match(widgets, /widget-window-dragging \{ z-index: 5;/)
